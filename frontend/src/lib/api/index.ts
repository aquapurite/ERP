...
export const fixedAssetsApi = {
  ...
  capex: {
    list: async (params?: {
      ...
      const { data } = await apiClient.get<{ items: CapexRequest[]; total: number; page: number; size: number; pages: number }>('/fixed-assets/capex', { params });
      return data;
    },
    getById: async (id: string) => {
      const { data } = await apiClient.get<CapexRequest>(`/fixed-assets/capex/${id}`);
      return data;
    },
    create: async (capex: {
      request_date: string;
      ...
      const { data } = await apiClient.post<CapexRequest>('/fixed-assets/capex', capex);
      return data;
    },
    update: async (id: string, capex: Partial<{
      request_date: string;
      asset_category_id: string;
      asset_name: string;
      description: string;
      ...
      const { data } = await apiClient.put<CapexRequest>(`/fixed-assets/capex/${id}`, capex);
      return data;
    },
    delete: async (id: string) => {
      await apiClient.delete(`/fixed-assets/capex/${id}`);
    },
    submit: async (id: string) => {
      const { data } = await apiClient.post<CapexRequest>(`/fixed-assets/capex/${id}/submit`);
      return data;
    },
    approve: async (id: string) => {
      const { data } = await apiClient.post<CapexRequest>(`/fixed-assets/capex/${id}/approve`);
      return data;
    },
    reject: async (id: string, reason: string) => {
      const { data } = await apiClient.post<CapexRequest>(`/fixed-assets/capex/${id}/reject`, { reason });
      return data;
    },
    createPO: async (id: string, poData: { vendor_id: string; expected_delivery_date?: string; terms_and_conditions?: string }) => {
      const { data } = await apiClient.post<CapexRequest>(`/fixed-assets/capex/${id}/create-po`, poData);
      return data;
    },
    receive: async (id: string, receiveData: { grn_id: string; received_date: string }) => {
      const { data } = await apiClient.post<CapexRequest>(`/fixed-assets/capex/${id}/receive`, receiveData);
      return data;
    },
    capitalize: async (id: string, capitalizeData: {
      capitalization_date: string;
      actual_cost: number;
      installation_cost?: number;
      other_costs?: number;
      ...
      const { data } = await apiClient.post<CapexRequest>(`/fixed-assets/capex/${id}/capitalize`, capitalizeData);
      return data;
    },
    getDashboard: async () => {
      const { data } = await apiClient.get<CapexDashboard>('/fixed-assets/capex/dashboard/stats');
      return data;
    },
  },
};
...
export interface CapexDashboard {
  ...
  total_estimated_this_year: number;
  total_approved_this_year: number;
  total_capitalized_this_year: number;
  pending_approval_amount: number;
  category_wise: Array<{ category: string; count: number; amount: number }>;
...
export const expensesApi = {
  // Dashboard
  getDashboard: async () => {
    const { data } = await apiClient.get<ExpenseDashboard>('/expenses/dashboard/stats');
    return data;
  },

  // Categories
  listCategories: async (params?: { page?: number; size?: number; is_active?: boolean; search?: string }) => {
    const { data } = await apiClient.get<{ items: ExpenseCategory[]; total: number; page: number; size: number; pages: number }>('/expenses/categories', { params });
    return data;
  },
  getCategoryDropdown: async () => {
    const { data } = await apiClient.get<Array<{ id: string; code: string; name: string; gl_account_id?: string; requires_receipt: boolean }>>('/expenses/categories/dropdown');
    return data;
  },
  getCategory: async (id: string) => {
    const { data } = await apiClient.get<ExpenseCategory>(`/expenses/categories/${id}`);
    return data;
  },
  createCategory: async (category: {
    ...
    const { data } = await apiClient.post<ExpenseCategory>('/expenses/categories', category);
    return data;
  },
  updateCategory: async (id: string, category: Partial<{
    name: string;
    description: string;
    gl_account_id: string;
    requires_receipt: boolean;
    ...
    const { data } = await apiClient.put<ExpenseCategory>(`/expenses/categories/${id}`, category);
    return data;
  },
  ...
  listVouchers: async (params?: {
    ...
    const { data } = await apiClient.get<{ items: ExpenseVoucher[]; total: number; page: number; size: number; pages: number }>('/expenses', { params });
    return data;
  },
  getVoucher: async (id: string) => {
    const { data } = await apiClient.get<ExpenseVoucher>(`/expenses/${id}`);
    return data;
  },
  ...
  updateVoucher: async (id: string, voucher: Partial<{
    voucher_date: string;
    expense_category_id: string;
    amount: number;
    gst_amount: number;
    ...
    const { data } = await apiClient.put<ExpenseVoucher>(`/expenses/${id}`, voucher);
    return data;
  },
  ...
  submitVoucher: async (id: string) => {
    const { data } = await apiClient.post<ExpenseVoucher>(`/expenses/${id}/submit`);
    return data;
  },
  approveVoucher: async (id: string) => {
    const { data } = await apiClient.post<ExpenseVoucher>(`/expenses/${id}/approve`);
    return data;
  },
  rejectVoucher: async (id: string, reason: string) => {
    const { data } = await apiClient.post<ExpenseVoucher>(`/expenses/${id}/reject`, { reason });
    return data;
  },
  postVoucher: async (id: string) => {
    const { data } = await apiClient.post<ExpenseVoucher>(`/expenses/${id}/post`);
    return data;
  },
  payVoucher: async (id: string, payment_reference: string, payment_mode?: string, bank_account_id?: string) => {
    const { data } = await apiClient.post<ExpenseVoucher>(`/expenses/${id}/pay`, { payment_reference, payment_mode, bank_account_id });
    return data;
  },
  addAttachment: async (id: string, attachment: { file_url: string; file_name: string; file_type?: string; file_size?: number }) => {
    const { data } = await apiClient.post<ExpenseVoucher>(`/expenses/${id}/attachments`, attachment);
    return data;
  },
};
