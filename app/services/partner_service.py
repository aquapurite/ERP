...
class PartnerService:
    ...
    async def register_partner(
        ...
        if data.bank_account_number:
            ...
            partner.bank_account_name = data.bank_account_name
        ...
        if referred_by_id:
            referral = PartnerReferral(
                ...
                referral_bonus=Decimal("0"),  # Bonus awarded after first order
                is_qualified=False,
            )
    ...
    async def submit_kyc(
        ...
        if data.pan_number:
            partner.pan_number = data.pan_number
            partner.pan_card_url = data.pan_card_url

        if data.selfie_url:
            partner.selfie_url = data.selfie_url
        ...
        if data.cancelled_cheque_url:
            partner.cancelled_cheque_url = data.cancelled_cheque_url

        # Update status
        partner.kyc_status = "PENDING"
        partner.status = "KYC_SUBMITTED"
        partner.kyc_submitted_at = datetime.now(timezone.utc)
    ...
    async def get_commission_summary(self, partner_id: uuid.UUID) -> dict:
        ...
        total_result = await self.db.execute(
            select(func.coalesce(func.sum(PartnerCommission.net_amount), 0))
    ...
    async def create_payout(
        ...
        net_amount = sum(c.net_amount for c in approved_commissions)
        ...
        payout = PartnerPayout(
            ...
            payout_details={
                "bank_account": partner.bank_account_number[-4:],  # Last 4 digits
    ...
    async def check_tier_upgrade(self, partner_id: uuid.UUID) -> Optional[PartnerTier]:
        ...
        # Get all tiers ordered by commission rate (ascending = lower tiers first)
        tiers_result = await self.db.execute(
            select(PartnerTier)
            .where(PartnerTier.is_active == True)
            .order_by(PartnerTier.commission_rate.desc())  # Highest tier first
        )
    ...
    async def get_tier_progress(self, partner_id: uuid.UUID) -> dict:
        ...
        if current_tier:
            next_tier_result = await self.db.execute(
                ...
                .where(
                    ...
                    PartnerTier.commission_rate > current_tier.commission_rate
                )
        ...
        orders_progress = min(100, int((partner.total_orders / next_tier.min_orders) * 100)) if next_tier.min_orders > 0 else 100
        sales_progress = min(100, int(((partner.total_sales or Decimal("0")) / next_tier.min_revenue) * 100)) if next_tier.min_revenue > 0 else 100

        return {
            "current_tier": current_tier.name if current_tier else "None",
            "next_tier": next_tier.name,
            "orders_required": next_tier.min_orders,
            "orders_completed": partner.total_orders,
            "orders_progress": orders_progress,
            "revenue_required": float(next_tier.min_revenue),
            "revenue_completed": float(partner.total_sales or 0),
    ...
    async def get_partner_analytics(self) -> dict:
        ...
        total_sales_result = await self.db.execute(
            select(func.coalesce(func.sum(CommunityPartner.total_sales), 0))
        )
        ...
        month_sales_result = await self.db.execute(
            select(func.coalesce(func.sum(PartnerOrder.order_amount), 0))
            .where(PartnerOrder.created_at >= month_start)
        )
        ...
        top_partners_result = await self.db.execute(
            select(CommunityPartner.full_name, CommunityPartner.total_sales, CommunityPartner.total_orders)
            .where(CommunityPartner.status == "ACTIVE")
            .order_by(CommunityPartner.total_sales.desc())
            .limit(5)
        )
        top_partners = [
            {"name": r[0], "sales": float(r[1] or 0), "orders": r[2]}
